...
if(isset($_POST['add_to_cart'])){
   ...
   // !important this line of code assumes that there is a quantity products table
   $check_quantity = $conn->prepare("SELECT quantity FROM `products` WHERE name = ?");
   $check_quantity->execute([$p_name]);
   $quantity_result = $check_quantity->fetch(PDO::FETCH_ASSOC);
   $available_quantity = $quantity_result['quantity'];

   if ($available_quantity > 0) {
      // Update the available quantity in the database
      $new_quantity = $available_quantity - 1;
      $update_quantity = $conn->prepare("UPDATE `products` SET quantity = ? WHERE name = ?");
